; This .emacs file is a modified version of
; the .emacs from Casey Muratory 
...
(setq my-todo-file "c:/code/tfgame/code/todo.txt")
(setq my-log-file "c:/code/tfgame/code/log.txt")
...
(when my-win32 
  ...
  (setq my-font "outline-Liberation Mono")
)
...
(when my-aquamacs 
  ...
  (setq my-makescript "./build.macosx")
)
...
(setq ediff-window-setup-function 'my-ediff-setup-windows)
...
; CC++ mode handling
(defun my-big-fun-c-hook ()
  ; Set my style for the current buffer
  (c-add-style "BigFun" my-big-fun-c-style t)
  ...
  (defun my-header-format ()
     ...
     (insert "   $Date: $\n")
     (insert "   $Revision: $\n")
     (insert "   $Creator: Lawrence Ibarria $\n")
     (insert "   $Notice: (C) Copyright 2017 by Lawrence Ibarria. All Rights Reserved. $\n")
     (insert "   ======================================================================== */\n")
     (insert "\n")
     (insert "#define ")
     (push-mark)
  ...
  (cond ((file-exists-p buffer-file-name) t)
        ((string-match "[.]hin" buffer-file-name) (my-source-format))
        ((string-match "[.]cin" buffer-file-name) (my-source-format))
  ...
  (define-key c++-mode-map "\ec" 'my-find-corresponding-file)
  (define-key c++-mode-map "\eC" 'my-find-corresponding-file-other-window)
...
(defun my-replace-string (FromString ToString)
  "Replace a string without moving point."
  (interactive "sReplace: \nsReplace: %s  With: ")
  (save-excursion
    (replace-string FromString ToString)
  ))
(define-key global-map [f8] 'my-replace-string)

(add-hook 'c-mode-common-hook 'my-big-fun-c-hook)
...
(defun my-big-fun-text-hook ()
  ...
  (define-key text-mode-map "\es" 'my-save-buffer)
  )
(add-hook 'text-mode-hook 'my-big-fun-text-hook)
...
(defun maximize-frame ()
    "Maximize the current frame"
     (interactive)
     (when my-aquamacs (aquamacs-toggle-full-frame))
     (when my-win32 (w32-send-sys-command 61488)))
...
(defun my-replace-in-region (old-word new-word)
...
(define-key global-map "\el" 'my-replace-in-region)

(define-key global-map "\eo" 'query-replace)
(define-key global-map "\eO" 'my-replace-string)
...
(defun my-never-split-a-window
    "Never, ever split a window.  Why would anyone EVER want you to do that??"
    nil)
(setq split-window-preferred-function 'my-never-split-a-window)
